	...
	//获取数组遍历次数
		...
		for ($i = 0; $i < $len; $i++) 
		{
			...
			{ 
				//dump($arr);
				//去除多余的循环次数
				$array[] = array_unique(['menu_id'=>$arr[0],'shop_id'=>$arr[1]]);
				
				
				$shop_id = $array[0]['shop_id'];//商铺id
				$menu_id = $array[0]['menu_id'];//商铺id
				//dump($shop_id);
				//根据商品id与商家id,查询商品详情
				$data = $this->order->select_goods_detail($array[0]['menu_id'],$array[0]['shop_id']);
				
				$menu_price[] = $data[$j]['menu_price'];
					
			}
		}
		$shops_id	= $shop_id;
		$menus_id	= $menu_id;
		$menus_num	= $menu_num[0]['num'];
		...
		$this->assign('menus_num',$menus_num);
		return $this->fetch();	
	}
